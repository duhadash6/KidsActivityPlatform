      ...
      <form method="dialog">
        <div class="grid grid-cols-6 gap-6">
          ...
          <div class="col-span-6 sm:col-span-3">
            ...
            <input type="number" v-model="remise" id="product-remise" class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-cyan-600 focus:border-cyan-600 block w-full p-2.5" placeholder="Remise" required>
          </div>
          <div class="col-span-6 sm:col-span-3">
        ...
        <div class="p-6 border-t border-gray-200 rounded-b">
          <button @click="updateOffre" class="text-white bg-gradient-selected font-medium rounded-lg text-base inline-flex items-center px-3 py-2.5 text-center mr-2">Mettre à jour l'offre</button>
        </div>
        <div class="p-6 border-t border-gray-200 rounded-b">
...
export default {
  ...
  methods: {
    async updateOffre() {
        try {
          const response = await axios.put(`/admin/offres/${this.offre.offre.idOffre}`, {
            titre: this.form.titre,
            description: this.form.description,
            dateDebutOffre: this.form.dateDebutOffre,
            dateFinOffre: this.form.dateFinOffre,
            activites: this.form.activites,
          });
          if (response.status === 200) {
          this.$emit('offre-updated');
          alert('Offre editée avec succès ');
